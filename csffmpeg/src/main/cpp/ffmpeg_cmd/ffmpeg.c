...
static int do_video_stats(OutputStream *ost, int frame_size);
...
{
    LOGE("sigterm_handler + sig: %d", sig);
...
static int abort_codec_experimental(AVCodec *c, int encoder)
{
    return exit_program(1);
}
...
//                       ost->file_index, ost->st->index, ost->last_mux_dts, pkt->dts);
                LOGW("Non-monotonous DTS in output stream "
                       "%d:%d; previous: %"PRId64", current: %"PRId64"; ",
                       ost->file_index, ost->st->index, ost->last_mux_dts, pkt->dts);
                if (exit_on_error) {
                    LOGE( "aborting.\n");
...
//                       max);
                LOGW("changing to %"PRId64". This may result "
                       "in incorrect timestamps in the output file.\n",
                       max);
                if (pkt->pts >= pkt->dts)
...
static int output_packet(OutputFile *of, AVPacket *pkt, OutputStream *ost)
{
    int ret = 0;
    ...
    if (ost->nb_bitstream_filters) {
        ...
        while (idx) {
            ...
            if (idx < ost->nb_bitstream_filters) {
                ...
                int result = write_packet(of, pkt, ost, 0);
                if (result) {
                    return result;
                }
            }
...
static int flush_encoders(void)
{
    int i, ret;
    ...
    if (ist->decoding_needed) {
        ...
        if ((ret = avcodec_open2(ist->dec_ctx, codec, &ist->decoder_opts)) < 0) {
            if (ret == AVERROR_EXPERIMENTAL) {
                return abort_codec_experimental(codec, 0);
            }
        ...
        int result = assert_avoptions(ist->decoder_opts);
        if (result) {
            return result;
        }
    }
...
static int check_init_output_file(OutputFile *of, int file_index)
{
    ...
    for (i = 0; i < of->ctx->nb_streams; i++) {
        ...
        while (av_fifo_size(ost->muxing_queue)) {
            AVPacket pkt;
            av_fifo_generic_read(ost->muxing_queue, &pkt, sizeof(pkt), NULL);
            int result = write_packet(of, &pkt, ost, 1);
            if (result) {
                return result;
            }
        }
...
static int parse_forced_key_frames(char *kf, OutputStream *ost,
...
static int init_output_stream(OutputStream *ost, char *error, int error_len)
{
    ...
    if (ost->encoding_needed) {
        ...
        if ((ret = avcodec_open2(ost->enc_ctx, codec, &ost->encoder_opts)) < 0) {
            if (ret == AVERROR_EXPERIMENTAL)
                return abort_codec_experimental(codec, 1);
            snprintf(error, error_len,
...
//    }
    ...
    int result = received_nb_signals ? 255 : main_return_code;
    if (ffmpeg_cleanup)
        ffmpeg_cleanup(result);

    LOGE("exit_program ret=%d", result);
