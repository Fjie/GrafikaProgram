...
extern "C" {
    ...
    block_queue init_block_queue() {
        return block_queue();
    }

    void release_bq(block_queue queue) {
        delete queue;
    }

    void clear_bq(block_queue queue) {
        queue.clear();
    }

    int is_full_bq(block_queue queue) {
        if (queue.full()) return 0;
        else return -1;
    }

    int is_empty_bq(block_queue queue) {
